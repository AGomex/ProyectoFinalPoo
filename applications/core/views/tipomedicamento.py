...
from applications.core.forms.tipomedicamento import TipoMedicamentoForm
...
class TipoMedicamentoCreateView(PermissionMixin, CreateViewMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f"Éxito al crear el tipo medicamento {tipomedicamento.nombre}.")
        return response
...
class TipoMedicamentoUpdateView(PermissionMixin, UpdateViewMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        tipomedicamento = self.object
        messages.success(self.request, f"Éxito al actualizar el Tipo Medicamento  {tipomedicamento.nombre}.")
        return response
...
class TipoMedicamentoDeleteView(PermissionMixin, DeleteViewMixin, DeleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['description'] = f"¿Desea eliminar el Tipo Medicamento: {self.object.name}?"
        context['back_url'] = self.success_url
        return context
    ...
    def form_valid(self, form):
        ...
        TipoMedicamento_name = self.object.nombre
        
        # Llamar al delete del padre
        response = super().form_valid(form)
        
        # Agregar mensaje
        messages.success(self.request, f"Éxito al eliminar lógicamente el gastomensual {TipoMedicamento_name}.")
